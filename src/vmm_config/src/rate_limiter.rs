...
// The persist module will be local to vmm_config now.
pub mod persist;
// Use the moved utils::time module
use crate::utils::time::NANOS_PER_MILLISECOND;
...
#[cfg_attr(feature = "kani", kani::requires(x > 0 && y > 0))]
#[cfg_attr(feature = "kani", kani::ensures(
    |&result| result != 0
        && x % result == 0
        && y % result == 0
))]
...
impl TokenBucket {
    ...
    pub fn new(size: u64, one_time_burst: u64, complete_refill_time_ms: u64) -> Option<Self> {
        ...
        let complete_refill_time_ns =
            complete_refill_time_ms.checked_mul(NANOS_PER_MILLISECOND)?;
    ...
    fn auto_replenish(&mut self) {
        ...
        if time_delta >= u128::from(self.refill_time * NANOS_PER_MILLISECOND) {
            ...
            // In the constructor, we assured that (self.refill_time * NANOS_PER_MILLISECOND)
    ...
    pub fn reduce(&mut self, mut tokens: u64) -> BucketReduction {
        ...
        if tokens > self.budget {
            ...
            if tokens > self.size {
                // TODO: vmm_config does not have a logger. Replace with appropriate error handling.
                eprintln!(
        ...
        match TokenBucket::new(size, one_time_burst, complete_refill_time_ms) {
            None => assert!(
                size == 0
                    ...
                    || complete_refill_time_ms > u64::MAX / NANOS_PER_MILLISECOND
            ),
...
pub(crate) mod tests {
    ...
    fn test_token_bucket_preprocess() {
        let tb = TokenBucket::new(1000, 0, 1000).unwrap();
        assert_eq!(tb.get_processed_capacity(), 1);
        assert_eq!(tb.get_processed_refill_time(), NANOS_PER_MILLISECOND);
        ...
        assert_eq!(
            ...
            13 * 17 * (NANOS_PER_MILLISECOND / thousand)
        );
    }
