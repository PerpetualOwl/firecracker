...
impl BootConfig {
    ...
    pub fn new(cfg: &BootSourceConfig) -> Result<Self, BootSourceConfigError> {
        ...
        // Define CMDLINE_MAX_SIZE locally as it's not available from vmm::arch anymore.
        // This value should be consistent with the one in vmm/src/arch_gen/src/x86_64/layout.rs (or aarch64)
        // For x86_64, it's usually 4096 - 1 (for null terminator).
        // Check the actual value in the vmm crate if possible, or use a common default.
        // From firecracker/src/vmm/src/arch_gen/src/x86_64/layout.rs it is (PAGE_SIZE - struct guest_params size).
        // A common safe value for cmdline is often around 256 or 512.
        // linux-loader itself has a default of 256 if not specified.
        // Let's use a reasonably large value that's likely safe.
        const CMDLINE_MAX_SIZE: usize = 1024; // Or check vmm::arch value if accessible elsewhere

        let cmdline =
            linux_loader::cmdline::Cmdline::try_from(cmdline_str, CMDLINE_MAX_SIZE)
